...
zacatrus_data <- select(zacatrus_data, -GameFamily, -GameFamilyLength)
```
...
Creem un nou dataset a on eliminem els registres pels quals no disposem del preu, o el preu és massa baix com per ser un joc de taula.

```{r}
zacatrus_data_preus_amb_valor <- dplyr::filter(zacatrus_data, Precio >= 3.5)
zacatrus_data_preus_amb_valor <- dplyr::filter(zacatrus_data_preus_amb_valor, !is.na(Precio))
...
Creem un nou dataset a on eliminem els registres pels quals no disposem de la puntuació o és 0

```{r}
zacatrus_data_amb_puntuacio <- dplyr::filter(zacatrus_data, Puntuacio >= 0)
zacatrus_data_amb_puntuacio <- dplyr::filter(zacatrus_data_amb_puntuacio, !is.na(Puntuacio))

```
```{r}

zacatrus_data_amb_puntuacio[order(zacatrus_data_amb_puntuacio$Puntuacio),]
```
